...
alter table public.orders
  add column if not exists utm_attribution jsonb
  not null default '{}'::jsonb;

comment on column public.orders.utm_attribution is
'Last-touch UTM attribution captured by the storefront at checkout.';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'orders_utm_attribution_object_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_utm_attribution_object_check
      check (jsonb_typeof(utm_attribution) = 'object');
  end if;
end;
$constraint$;
...
as $$
begin
  return public.create_store_order_idempotent(
    p_client_request_id,
    p_customer,
    p_items,
    p_coupon_code,
...
revoke all on function
public.create_store_order(jsonb, jsonb, text, jsonb)
from public;
revoke all on function
public.create_store_order(jsonb, jsonb, text)
from public;
